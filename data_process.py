...
# construct multi topological graph
def target_to_graph(target_key, target_sequence, contact_dir, aln_dir,dataset):
    ...
    orig_target_edge_index = target_edge_index.copy()

    f = open('data/' + dataset + '/pocket/' + target_key + '.json')
    p = json.load(f)
    is_stub = np.zeros(target_feature.shape[0])
    for i in p.keys():
        p_pos = p[i]
        stub = np.zeros(target_feature.shape[1])
        stub = stub.reshape(1, -1)
        target_feature = np.concatenate((target_feature, stub), axis=0)
        is_stub = np.concatenate((is_stub, [1]), axis=0)
        new_edge = np.array([[]])
        init = True
        for pos in p_pos:
            if init:
                new_edge = np.array([[pos, target_size]])
                init = False
                continue
            temp = np.array([[pos, target_size]])
            new_edge = np.concatenate((temp, new_edge), axis=0)
        new_edge = torch.LongTensor(new_edge)
        target_size = target_size + 1
        target_edge_index = np.concatenate((target_edge_index, new_edge), axis=0)
    return target_size, target_feature, target_edge_index, is_stub, orig_target_edge_index
...
def create_dataset(dataset, fold=0, tune=False):
    # load dataset
    ...
    if tune:
        ...
        train_folds = []
        valid_fold = json.load(open(dataset_path + 'folds/test_fold_setting1.txt'))
    ...
    for key in target_key:
        if not valid_target(key, dataset):  # ensure the contact and aln files exists
            continue
        g = target_to_graph(key, proteins[key], contac_path, msa_path,dataset)
        target_graph[key] = g
